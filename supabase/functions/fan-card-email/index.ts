// Supabase Edge Function: fan-card-email
...
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Fan Card Application</title></head>
<body style="margin:0; padding:0; background:#ffffff; font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; font-size:14px; line-height:1.5; color:#18181b;">
  <div style="max-width:600px; margin:0 auto; padding:24px;">
    <p style="margin:0 0 4px; font-size:13px; color:#71717a;">Fan Card</p>
    <h1 style="margin:0 0 8px; font-size:24px; font-weight:600;">Fan Card Membership Application</h1>
    <p style="margin:0 0 24px; font-size:14px; color:#71717a;">A new fan card application was submitted from the website.</p>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%; border-collapse:collapse; border:1px solid #e4e4e7; border-radius:12px; overflow:hidden;">
      <tbody>
        ...
        ${row("Mailing Address", data.address)}
        ${row("City", data.city)}
        ${row("State / Province", data.state)}
        ${row("Country", data.country)}
        ${row("ZIP / Postal", data.zip)}
        ${row("Membership Level", data.level)}
        ${row("Favorite role / movie", data.favoriteRole)}
        ${row("How did you hear about us?", data.heardVia, true)}
      </tbody>
    </table>
...
serve(async (req: Request) => {
  ...
  const allowEmpty = new Set(["heardVia"]);
